...
bunk_choice = int(input("These nuclear codes are stored in an extremely secure bunker. You can either breach the bunker by force or infiltrate it. (1/2) "))
if guard([1,2], bunk_choice) != True:
    while guard([1,2], bunk_choice) != True:
        print("invalid.")
...
if bunk_choice == 1:
    ...
    flashvar = int(input(f"{team_name} arrives at the site of the bunker. They'll need to take out the small army of guards in the front. " \
                         "Should they throw a flashbang to distract the guards, or attempt to gun them down? (1/2) "))
    if guard([1,2], flashvar) != True:
        while guard([1,2], flashvar) != True:
            print("invalid.")
            flashvar = int(input(f"{team_name} arrives at the site of the bunker. They'll need to take out the small army of guards in the front. " \
                                "Should they throw a flashbang to distract the guards, or attempt to gun them down? (1/2) "))
    if flashvar == 1:
        ...
        if guard(alive_list, char_choice) != True:
            print("invalid.")
            while guard(alive_list, char_choice) != True:
                print("invalid.")
                char_choice = int(input(f"Who should throw the flash? {alive_select()}"))
        for person in operative_list: 
        ...
        char_choice = int(input(f"Who should attempt to gun down the guards? {alive_select()}"))
        if guard(alive_list, char_choice) != True:
            while guard(alive_list, char_choice) != True:
                print("invalid.")
    ...
    code_choice = int(input("Should the team attempt to get the code themselves or hack into the vault? (1/2) "))
    if guard([1,2], code_choice) != True:
        while guard([1,2], code_choice) != True:
            print("invalid.")
    ...
    if code_choice == 1:
        ...
        pers_choice = int(input(f"Who should attempt the task? {alive_select()}"))
        if guard(alive_list, pers_choice) != True:
            while guard(alive_list, pers_choice) != True:
                print("invalid.")
        ...
        if guard(alive_list, ins_char) != True:
            while guard(alive_list, ins_char) != True:
                print("invalid.")
                ins_char = input(f"Who should hack into the system? {alive_select()}")
        for person in operative_list: 
    ...
    for person in operative_list:
        if person.captured == False:
            person.skills_showcase(1)
    helichoice = int(input(f"Who should attempt the task? {alive_select()}"))
    if guard(alive_list, char_choice) != True:
            while guard(alive_list, helichoice) != True:
                print("invalid.")
    ...
    ins_char = int(input(f"Who should be the mole? {alive_select()}"))
    if guard(alive_list, ins_char) != True:
        while guard(alive_list, ins_char) != True:
            print("invalid.")
    ...
    for person in operative_list:
        if person.captured == False:
            person.skills_showcase(2)
    ins_char = int(input(f"Who should hack into the system? {alive_select()}"))
    if guard(alive_list, ins_char) != True:
        while guard(alive_list, ins_char) != True:
            print("invalid.")
            ins_char = input(f"Who should hack into the system? {alive_select()}")
